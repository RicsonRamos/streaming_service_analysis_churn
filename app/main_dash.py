...
if model and not df_raw.empty:
    ...
    if show_shap:
        ...
        with col_shap1:
            ...
            st.write(
                f"**Probabilidade de Churn:** {df_raw.loc[customer_idx, 'Churn_Probability']:.2%}"
            )
    ...
    st.info(
        "Esta lista contém os dados identificáveis para que o time de Customer Success possa realizar o contato."
    )
    ...
    st.dataframe(
        high_risk_display[cols_view]
        .style.format({"Churn_Probability": "{:.2%}", "Monthly_Spend": "R$ {:.2f}"})
    ...
    st.warning(
        "⚠️ Aguardando pipeline de dados: Verifique se o modelo foi treinado e o MLflow está acessível em http://mlflow:5000"
    )
