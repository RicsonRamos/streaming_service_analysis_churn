...
class FeatureEngineer:
    ...
    def __init__(self, cfg: dict):
        ...
        self.schema: dict[str, Any] = cfg.get("feature_schema", {})
        self.target: Optional[str] = self.schema.get("target")
        self.drop_cols: list[str] = self.schema.get("drop_columns", [])
        self.params: dict[str, Any] = {}  # TIPAGEM CORRETA
        ...
        for col in cat_features:
            if col in X.columns:
                ...
                X[col] = pd.Categorical(
                    X[col],
                    categories=self.params.get("categories", {}).get(col),
                )
