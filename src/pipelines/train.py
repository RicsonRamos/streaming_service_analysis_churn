...
from src.models.xgboost import (
    ChurnXGBoost,  # Nome da classe corrigido conforme seu erro
)
...
class TrainingPipeline:
    ...
    def run(self, tune: bool = False):
        ...
        with mlflow.start_run() as run:
            ...
            logger.info(
                f"Split Temporal Aplicado: Treino (0 a {split_idx}) | Teste ({split_idx} a {len(df_clean)})"
            )
            ...
            model_wrapper.save(path=self.cfg["artifacts"]["current_model"], artifact=artifact)
